...
class Thecore::Generate < Thor
  package_name "Generate"
  map "-L" => :list
  
  desc "Thecorize the Model applying all the sane defaults proved efficient during the years of development using Thecore in production"   # [4]
  # method_options :force => :boolean, :entry => :string           # [5]
  def models
    loop do
      ...
      # API
      FileUtils.copy "/etc/thecore/templates/model_api_concern.tt", "app/models/concerns/api/#{filename}" unless File.exists?("app/models/concerns/api/#{filename}")
      FileUtils.copy "/etc/thecore/templates/model_rails_admin_concern.tt", "app/models/concerns/rails_admin/#{filename}" unless File.exists?("app/models/concerns/rails_admin/#{filename}")
    ...
    polymorphics.each do |polymorphic_belongs_to|
      ...
      choices = @model_files.reject {|m| m == entry || has_polymorphic_has_many?(File.join(@plugin_models_dir,m), polymorphic_target_association)}
      answers = ask_question_multiple_choice choices, "Where do you want to add the polymorphic has_many called #{polymorphic_target_association} found in #{entry}?"
        ...
        answers.each do |answer|
          inject_into_file File.join("app/models", answer), after: "# Associations\n" {"  has_many :#{entry.split(".").first.split("/").last.pluralize}, as: :#{polymorphic_target_association}, inverse_of: :#{answer.split(".").first.split("/").last.singularize}, dependent: :destroy"} if File.exists?(File.join("app/models", answer))
        end
      end
      ...
      manies.each do |has_many|
        ...
        if !File.exists?(associated_file)
          ...
          FileUtils.copy "/etc/thecore/templates/association_concern.tt", "config/initializers/#{initializer_name}" unless File.exists?(File.join("config/initializers", initializer_name))
    ...
    def is_api_app? file
      ...
      (File.readlines(file).grep(/^  spec.add_dependency 'model_driven_api'/).size > 0) rescue false
    end
    
    def is_gui_app? file
      # Must be not commented out so the ^  part
      (File.readlines(file).grep(/^  spec.add_dependency 'thecore_ui_rails_admin'/).size > 0) rescue false
    end
    ...
    def has_rails_admin_declaration?(file)
      File.readlines(file).grep(Regexp.new("^ *rails_admin do")).size > 0 rescue false
    end
    ...
    def has_has_many_association? file, assoc
      (File.readlines(file).grep(Regexp.new("^ +has_many +:#{assoc}")).size > 0) rescue false
    end
